...
public class GameStatusList {
    ...
    static GameStatus afterTurn2 = opponentIsBreaker().turn(PLAYER).removeBalls(4, 9).currentPlayerColor(SOLIDS).build();
    static GameStatus afterTurn3 = opponentIsBreaker().turn(OPPONENT).removeBalls(1, 2, 3, 4, 5, 5, 6, 7, 9).currentPlayerColor(STRIPES).build();
    static GameStatus afterTurn4 = playerBreaking();
    static GameStatus afterTurn5 = playerIsBreaker().turn(OPPONENT).removeBalls(1, 2).currentPlayerColor(STRIPES).build();
    static GameStatus afterTurn6 = opponentBreaking();
    static GameStatus afterTurn7 = opponentIsBreaker().turn(PLAYER).reBreak().build();
    static GameStatus afterTurn8 = opponentIsBreaker().turn(PLAYER).newGame().build();
    static GameStatus afterTurn9 = opponentIsBreaker().turn(OPPONENT).currentPlayerColor(SOLIDS).build();
    static GameStatus afterTurn10 = playerBreaking();
    static GameStatus afterTurn11 = opponentBreaking();
    static GameStatus afterTurn12 = opponentIsBreaker().turn(PLAYER).currentPlayerColor(STRIPES).removeBalls(10, 1).safetyLastTurn().build();
    static GameStatus afterTurn13 = opponentIsBreaker().turn(OPPONENT).currentPlayerColor(SOLIDS).removeBalls(10, 1).safetyLastTurn().build();
    static GameStatus afterTurn14 = playerBreaking();
    static GameStatus afterTurn15 = playerIsBreaker().turn(OPPONENT).reBreak().build();
